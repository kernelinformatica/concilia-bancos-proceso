...
class Conciliador:
    ...
    def detectar_motor(self, stream: BytesIO):
        try:
            if stream is None:
                raise ValueError("Stream de archivo inválido (None)")
            ...
            if header[:2] == b'PK':
                return 'openpyxl'
            logger.warning(f"Encabezado inesperado: {header}. Usaré 'openpyxl' por defecto.")
            return 'openpyxl'
        except Exception as e:
    ...
    def guardarUnicosEmpresa(self, unicos_empresa, cuenta_concilia):
        ...
        try:
            ...
            for row in df.itertuples():
                valores.append((
                    self.id_tipo_concilia, self.id_empresa, row.m_ingreso, row.comprobante, numerador, row.m_asiento, row.m_pase,
                    row.importe, row.m_minuta, row.concepto, row.detalle, fecha_actual, 'N', row.plan_cuentas, cuenta_concilia, self.id_usuario, row.c4, 1, row.padron_codigo
                ))
            sql = """
    ...
    def guardarTotalesBanco(self, resultado_totales_banco, cuenta_concilia):
        ...
        try:
            ...
            for row in df.itertuples():
                valores.append((
                    self.id_tipo_concilia, self.id_empresa, numerador, fecha, row.Index, row.importe, str(fecha_actual), 'N', 0, 0, self.id_usuario, 1
                ))
            sql = """
    ...
    def guardaResultadosConciliacion(self, resultado_concilia, cuenta_concilia =0):
        ...
        try:
            ...
            try:
                ...
                return {"codigo":200, "control":"OK", "mensaje": "Proceso de conciliación se completo con éxito. ("+str(cursor.rowcount)+") registros insertados."}
            except Exception as e:
